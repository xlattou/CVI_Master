...
//	 data->AOP[0]=data->AOP[0]-my_struTxCal.power[channel];
	return 0;
...
int tuningWavelength_FineTuning(int channel, struct struTestData *data,int index)		//波长微调
{
	...
	{	
		...
		else
		{
			...
			{
				DAC_Array[index]--;
				if(DAC_Array[index]<0x600)
				{
					DAC_Array[index]=0x600;
				}
			}
			else
			{
				DAC_Array[index]++;
				if(DAC_Array[index]>0xF00)
				{
					DAC_Array[index]=0xF00;
				}
			}
			
		}   
		count++;
	}while(count<20 && okFlag==FALSE);
	...
	data->TecDAC[index]=WL_Array[index];
