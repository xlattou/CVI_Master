...
int __stdcall I2C_HOST_RESET_F320_DLL (int iIndex, int F320I2CRate, int F320I2CSTOPbeforeReSTART); 
...
//INT16U	tec_Current;	               	//pStrA2[108~109]	//Tec Current TWDM Add
//INT16U	ld_Temp;						//pStrA2[106~107]	//LD Temperature TWDM Add;
INT8U	reserved3[4];					
...
/*A2 Table 2*/
/////////////////////////////////////////////////////////////////
struct A2Table2DitheringDef
{
    INT8U   TunableDWDMStepEnable:1;        // 1--enable  
    INT8U   TunableDWDMChEnable:1;          // 1--enable
    INT8U   TxDitherSupported:1;            // 1--supporte
    INT8U   REV0:5;		 
};  

struct A2Table2TxDitherDef
{
    INT8U   TxDitherEnable:1;            // 1--disables Dither
    INT8U   REV0:7;
};

struct A2Table2CurrentStatusDef
{
    INT8U   REV0:4;  
    INT8U   TxTune:1;                       // Identifies Tx is not ready due to tuning
    INT8U   WavelengthUlocked:1;            // Wavelength Unlocked Condition
    INT8U   TECFault:1;                     // TEC Fault
    INT8U   REV1:1;                         // 
};

struct A2Table2LatchStatusDef
{
    INT8U   REV0:2;
    INT8U   L_UnsupportedTXDither:1;        // Latched Unsupported TX Dither Request
    INT8U   L_NewChannel:1;                 // Latched New Channel Acquired
    INT8U   L_BadChannel:1;                 // Latched Bad Channel Requested
    INT8U   L_WavelengthUnlocked:1;         // Latched Wavelength Unlocked Condition
    INT8U   L_TECFault:1;                   // Latched TEC Fault
    INT8U   REV1:1;         
};
...
struct strA2Table2         
{
	INT8U								FirstByte;   
	INT8U								REVLOWE[127];
    struct A2Table2DitheringDef 		Dithering;  	// A2 Table2[128]  
    INT8U   							REV0[3];        // A2 Table2[129-131]
    INT16U  							LFL1;           // A2 Table2[132-133],Lasers First Frequency (THz)
    INT16U  							LFL2;           // A2 Table2[134-135],Lasers First Frequency (GHz*10), in units of 0.1 GHz
    INT16U  							LFH1;           // A2 Table2[136-137],Lasers Last Frequency (THz)
    INT16U  							LFH2;           // A2 Table2[138-139],Lasers Last Frequency (GHz*10), in units of 0.1 GHz
    INT16U  							LGrid;          // A2 Table2[140-141],Laser’s minimum supported grid spacing
                                            			// (GHz*10), i.e., in units of 0.1 GHz
                                            			// NOTE: LGrid can be a positive or negative number.
    INT8U   							REV1[2];        // A2 Table2[142-143]
    INT8U   							ChNumSet[2];    // A2 Table2[144-145],User input of wavelength channel integer 1 to N (N=Number of channels)
    INT8U	  							WaveLengthSet;  // A2 Table2[146-147],User input of Wavelength setpoint.(Units of 0.05 nm)
    INT8U   							REV2[3];        // A2 Table2[148-150]
    struct A2Table2TxDitherDef 			TxDither;       // A2 Table2[151], 
    INT16U  							FreqErr;        // A2 Table2[152-153],Frequency error reported in 16 bit signed integer with LSB=0.1 GHz
    INT16U  							WaveLengthErr;  // A2 Table2[154-155],Wavelength error reported in 16 bit signed integer with LSB=0.005 nm
    INT8U   							REV3[12];       // A2 Table2[156-167]
    struct A2Table2CurrentStatusDef		CurrentStatus;  // A2 Table2[168]   
    INT8U   							REV4[3];        // A2 Table2[169-171]
    struct A2Table2LatchStatusDef 		LatchStatus;    // A2 Table2[172]
    INT8U   							REV5[83];       // A2 Table2[173-255]
}; 

#define A2TAB2_SIZE     256
union uA2Table2
{
	struct strA2Table2 sStrA2Table2;
	INT8U  pStrA2Table2[A2TAB2_SIZE];  
};
